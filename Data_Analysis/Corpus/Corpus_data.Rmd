...
## Convert textpos in list into character
```{r}
textpos_new <- flatten_chr(textpos)
```

## convert textpos into a single character vector that 'kwic' can be applied
```{r}
tagged <- sapply(textpos_new, function(x){
  x <- paste0(x, collapse = " ")
  x <- str_squish(x)
})
```

## extract the designated concordance
```{r basekwic2, message=F, warning=F}
kwic_natural <- kwic(x = tagged, pattern = "accepted", window = 10) 
head(kwic_natural)
nrow(kwic_natural)
...
```{r basekwic2, message=F, warning=F}
kwic_natural_longer <- kwic(x = my_data, pattern = "accepted", window = 10)
